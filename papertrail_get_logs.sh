...
for HOUR in {0..23}; do
  if [[ $HOUR -lt 10 ]]; then
    curl --no-include -o ./$DATE-$MONTH-$YEAR/$YEAR-$MONTH-$DATE-0$HOUR.tsv.gz -L -H "X-Papertrail-Token: $PAPERTRAIL_TOKEN" \
        https://papertrailapp.com/api/v1/archives/$YEAR-$MONTH-$DATE-0$HOUR/download
  else
    curl --no-include -o ./$DATE-$MONTH-$YEAR/$YEAR-$MONTH-$DATE-$HOUR.tsv.gz -L -H "X-Papertrail-Token: $PAPERTRAIL_TOKEN" \
